...
def smoltalk2():
    import pandas as pd
    data_paths = [
        'data/datasets/multi_turn_reasoning_if_think-00000-of-00001.parquet',
        'data/datasets/smoltalk_everyday_convs_reasoning_Qwen3_32B_think-00000-of-00001.parquet',
        'data/datasets/smoltalk_systemchats_Qwen3_32B_think-00000-of-00001.parquet'
    ]

    data = []
    for fpath in data_paths:
        df = pd.read_parquet(fpath)['messages'].to_list()
        data.extend([{'messages': list(d), 'source': fpath.split('/')[-1]} for d in df])
...
def mot_math():
    def data_map(data):
        data['messages'] = [{
            "role": "system", 
            "content": "You are a helpful AI assistant." + random.choice([" ", "\n", "\n\n"]) + random.choice(THINK_STRINGS)
        }] + data['messages']

        return {
            "messages": data['messages'],
            # "conversations": tokenizer.apply_chat_template(data['messages'], tokenize=False, add_generation_prompt=False),
            "source": "open-r1/Mixture-of-Thoughts"
        }

    mot = load_dataset("open-r1/Mixture-of-Thoughts", "math")['train']
    mot = mot.map(data_map)
    return mot
...
if __name__ == "__main__":
    ...
    for stage, dataset in [("train", train_ds), ("test", test_ds)]:
        ...
        save_path = f"data/datasets/Smollm2_base_{stage}_think_v0.jsonl"
