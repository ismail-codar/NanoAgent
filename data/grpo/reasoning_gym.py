...
from .verifiers import response_judge, is_think, remove_thinking_section
import string
...
def generate_think_kshot():
    ...
    return [
        ...
        {"role": "assistant", "content": "<think>This is a math problem. Let's solve it in pairs.\n - 4 + 6 = 10\n - 10 - 1 = 9\n\nSo, answer is 9</think>\n\nThe answer is 9."},
    ]
...
def needle_haystack_parser(llm_gen, entry, score_fn, think):
    if think:
        if not is_think(llm_gen): return 0
    ...
    if ans in names:
        ...
        score = (1 / len(names[p:]))
    if score > 0:
        judge_score = response_judge(entry['question'], response=llm_gen, n_tokens=JUDGE_TOKENS, ref_answer=entry['answer'], strict_level=JUDGE_SCRICT_LEVEL)[1]
        score *= judge_score
    return min(score, 1)
...
def needle_haystack(tokenizer, size=500, prompt_token_len=None, think=False):
    ...
    for data in dataset:
        ...
        dataset_list.append({
            ...
            'answer': data['answer'],
            'scorer': partial(needle_haystack_parser, entry=data, score_fn=score_fn, think=think)
        })
...
def syllogism_parser(llm_gen, entry, score_fn, think):
    if think:
        if not is_think(llm_gen): return 0
        llm_gen = remove_thinking_section(llm_gen)
    last_line = last_line_parser(llm_gen)
...
def syllogism(tokenizer, size=500, prompt_token_len=None, think=False):
    ...
    for data in dataset:
        ...
        dataset_list.append({
            ...
            'answer': data['answer'],
            'scorer': partial(syllogism_parser, entry=data, score_fn=score_fn, think=think)
        })
...
def alice_in_wonderland_parser(llm_gen, entry, score_fn, think):
    if think:
        if not is_think(llm_gen): return 0
        llm_gen = remove_thinking_section(llm_gen)
    last_line = last_line_parser(llm_gen)
...
def alice_in_wonderland(tokenizer, size=500, prompt_token_len=None, think=False):
    ...
    for data in dataset:
        ...
        dataset_list.append({
            ...
            'answer': data['answer'],
            'scorer': partial(alice_in_wonderland_parser, entry=data, score_fn=score_fn, think=think)
        })
...
def gsm_symbolic_parser(llm_gen, entry, score_fn, think):
    if think:
        if not is_think(llm_gen): return 0
        llm_gen = remove_thinking_section(llm_gen)
    last_line = last_line_parser(llm_gen)
...
def gsm_symbolic(tokenizer, size=500, prompt_token_len=None, think=False):
    ...
    for data in dataset:
        ...
        dataset_list.append({
            ...
            'answer': data['answer'],
            'scorer': partial(gsm_symbolic_parser, entry=data, score_fn=score_fn, think=think)
        })
...
def chain_sum_parser(llm_gen, entry, score_fn, think):
    if think:
        if not is_think(llm_gen): return 0
        llm_gen = remove_thinking_section(llm_gen)
    last_line = last_line_parser(llm_gen)
...
def chain_sum(tokenizer, size=500, prompt_token_len=None, think=False):
    ...
    for data in dataset:
        ...
        dataset_list.append({
            ...
            'answer': data['answer'],
            'scorer': partial(chain_sum_parser, entry=data, score_fn=score_fn, think=think)
        })
...
def acre_parser(llm_gen, entry, score_fn, think):
    if think:
        if not is_think(llm_gen): return 0
        llm_gen = remove_thinking_section(llm_gen)
    last_line = last_line_parser(llm_gen)
...
def acre(tokenizer, size=500, prompt_token_len=None, think=False):
    ...
    for data in dataset:
        ...
        dataset_list.append({
            ...
            'answer': data['answer'],
            'scorer': partial(acre_parser, entry=data, score_fn=score_fn, think=think)
        })
